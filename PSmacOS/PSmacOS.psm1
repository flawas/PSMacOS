...
$Script:PSModuleVersion = (Import-PowerShellDataFile -Path (Join-Path -Path $Script:PSModulePath -ChildPath "$Script:PSModuleName.psd1"))['ModuleVersion']


## Module Loader

# Get and dot source all functions
Get-ChildItem -Path (Join-Path -Path $Script:PSModulePath -ChildPath 'Helpers'), (Join-Path -Path $Script:PSModulePath -ChildPath 'Functions') -Filter '*.ps1' -File -Recurse |
    ForEach-Object {
